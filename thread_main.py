def main():
    ...
    from app.configs.settings import (VERSION_SHEETS, VERSION_EXCEL, VERSION_TIME, VERSION_DELTA, SERVER_IP, PORT,
                                      nb_cols_before_split_sheets)
    from connect import Connect
    from analyser import convert_time_format, convert_seconds_to_time_format, convert_to_seconds, get_delta_time, \
        get_final_time, get_timer_phase, get_split_index, get_last_time
    ...
    def if_final_time(delta_list, time_list, save_time):
        s_index = len(delta_list) + nb_cols_before_split_sheets + 1
        if is_pb(s_index, save_time):
            write_time(delta_time=save_time, index_col=s_index)
            if VERSION in ["D+T+E", "D+T+S"]:
                write_time(time=save_time, index_col=s_index)
        else:
            write_time(delta_time=save_time, index_col=(s_index + 1))
            if VERSION in ["D+T+E", "D+T+S"]:
                write_time(time=save_time, index_col=(s_index + 1))
        delta_list.append(save_time)
        time_list.append(save_time)
        new_row()
        delta_list.clear()
        time_list.clear()
